...
def present_proof():
    ...
    if state == "request-received":
        ...
        check_proof = requests.get('http://10.132.0.2:8023/present-proof-2.0/records/' + pres_ex_id)
...
def test_propose():
    ...
    con_id_governament = "2604498a-4426-4dba-be42-baaf3a1efc77"
    did_user_bbs = "did:key:zUC7CfNKrbaitm1XG35W1t2MTaR7P4McHMUenmEURmX2jcRN6Qt7DLxRGcMs6vKfALE1sy4qegAk6n45Nj5LmBQXATaQ189hzHBwwegi1jH7DBFjeoPmZc3DiEuVmK35AGPhvMn"
    ...
    response = requests.post('http://10.132.0.2:8023/issue-credential-2.0/send-proposal', json=data)
...
def test_present():
    ...
    send_presentation = requests.post("http://10.132.0.2:8023/present-proof-2.0/records/" + pres_ex_id + "/send-presentation", json=data )
...
async def delete_credentials():
     #check_credentials = requests.get('http://10.132.0.2:8023/issue-credential-2.0/records')
    ...
    #for i in json_data:
        ...
        #dleted = requests.delete('http://10.132.0.2:8023/issue-credential-2.0/records/' + cred_ex_id )
        #print("DELETED -> " + dleted.text)#check_credentials = requests.get('http://10.132.0.2:8023/issue-credential-2.0/records')
    #print("GET issue-credential-2.0/records-> " + check_credentials.text)
    pass
...
async def get_credentials_w3c( did_governament_bbs):
    ...
    check_credentials = requests.post('http://10.132.0.2:8023/credentials/w3c', json=data)
...
async def delete_records_presentation_requests():
    ...
    check_proof = requests.get('http://10.132.0.2:8023/present-proof-2.0/records')
    json_data = json.loads(check_proof.text)['results']
    for i in json_data:
        pres_ex_id = str(i['pres_ex_id'])
        dleted = requests.delete('http://10.132.0.2:8023/present-proof-2.0/records/' + pres_ex_id )
...
async def get_presentation_proof_credential(pres_ex_id):
    ...
    fetch_cred = requests.get('http://10.132.0.2:8023/present-proof-2.0/records/' + pres_ex_id + '/credentials')
...
async def get_cred_def(cred_def_id):
    # get cred definition
    cred_def = requests.get('http://10.132.0.2:8023/credential-definitions/' + cred_def_id)

    return cred_def.text

async def get_schema(schema_id):
    #get transcript schema id
    schema = requests.get('http://10.132.0.2:8023/schemas/' + schema_id)
...
async  def create_did_bbs():    
    ...
    create_did_response = requests.post("http://10.132.0.2:8023/wallet/did/create", json=data)
...
async def chooseDID():
    ...
    did_list = requests.get("http://10.132.0.2:8023/wallet/did")
...
async def acceptConnection(con_id_governament, con_id_broker, con_id_other):
    ...
    if option == "1":
        ...
        receive_invite = requests.post('http://10.132.0.2:8023/connections/receive-invitation?alias=GovernamentInvitation', json=json.loads(invitation))
        ...
        invitation = input()
        receive_invite = requests.post('http://10.132.0.2:8023/connections/receive-invitation?alias=BrokerInvitation', json=json.loads(invitation))
        if debug: print("POST connections/receive-invitation -> " + receive_invite.text)
        ...
        invitation = input()
        receive_invite = requests.post('http://10.132.0.2:8023/connections/receive-invitation?alias=OtherInvitation', json=json.loads(invitation))
        if debug: print("POST connections/receive-invitation -> " + receive_invite.text)
        con_id_other = json.loads(receive_invite.text)['connection_id']
        print("Other Connection ID: " + con_id_other)

    elif option == "4":
        con_id_governament = con_id_governament
        con_id_broker = con_id_broker
        con_id_other = con_id_other
...
async def sendMessage(con_id, msg):
    response = requests.post("http://10.132.0.2:8023/connections/" + con_id + "/send-message", json={"content": msg})
...
async def propose_credential(con_id_governament, did_user_bbs, did_governament_bbs, input_url):
    ...
    response = requests.post('http://10.132.0.2:8023/issue-credential-2.0/send-proposal', json=proposal_json)
...
async def other_options(con_id_governament, con_id_broker, con_id_other):
    ...
    while True:
        ...
        if option == "1":
            ...
            cred_ex_id = input()
            response = requests.get('http://10.132.0.2:8023/issue-credential-2.0/records/' + cred_ex_id) 
...
async def run():
    ...
    #----------
    con_id_governament = "8d61af96-d52b-4946-90a5-14da65bd81f4"
    ...
    while True:
        ...
        if option == "1":
            ...
            fetch_cred = requests.get('http://10.132.0.2:8023/present-proof-2.0/records/' + pres_ex_id + '/credentials?count=30')
...
if __name__ == '__main__':  
    t1 = Thread(target=app.run, kwargs={'host': '0.0.0.0', 'port': 8026})
    t1.start()
    # Start Flask app  
