...
#---------------------------------
debug = False
con_id_constant_user = "4b35fbb4-6b30-4e8e-81fd-09803c4097b6"
did_broker = "did:key:zUC7CuDZAv2Z7T7PHsj4qrcnvRvfgym2Q1DFsapqHnhTSWAwMKzmUFRDqbSC5acu297hgn7SwMdBUZrxhqWdjBQgwv448Lc86uUe7S7ZdR9dcdQmntytpWPb58gKsSifxHPxUXM"
global t_start
global t_check
...
def handle_webhook():
    ...
    print("Received webhooks data: " + str(data))
...
async def get_public_did():
    did_public = requests.get("http://localhost:8011/wallet/did/public")
...
async def create_did_bbs():    
    ...
    create_did_response = requests.post("http://localhost:8011/wallet/did/create", json=data)
...
async def chooseDID():
    ...
    did_list = requests.get("http://localhost:8011/wallet/did")
...
async def addConnection():
    ...
    if option == "1":
        invitation = requests.post("http://localhost:8011/connections/create-invitation")
...
async def requestPresentation(con_id):
    ...
    response = requests.post("http://localhost:8011/present-proof-2.0/send-request", json=data)
...
async def sendMessage(con_id):
    ...
    msg = input()
    response = requests.post("http://localhost:8011/connections/" + con_id + "/send-message", json={"content": msg})

def verifyPresentation(pres_ex_id):
    response = requests.post("http://localhost:8011/present-proof-2.0/records/" + pres_ex_id + "/verify-presentation")
    print("Bruno: " + response.text)
